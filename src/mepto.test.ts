...
describe('array / object utilities', () => {
  ...
  it('$.each array', () => {
    let sum = 0;
    $.each([10, 20, 30], (_i: number, v: number) => { sum += v; });
    expect(sum).toBe(60);
  });

  it('$.each object', () => {
    const keys: string[] = [];
    $.each({ a: 1, b: 2 }, (k: string) => { keys.push(k); });
    expect(keys.length).toBe(2);
  });

  it('$.each early exit', () => {
    let stopped = 0;
    $.each([1, 2, 3], (i: number) => { stopped = i; if (i === 1) return false; });
...
describe('collection core', () => {
  ...
  it('.each()', () => {
    const ids: string[] = [];
    $('.child').each((_i: number, el: Element) => { ids.push(el.id); });
    expect(ids).toEqual(['fix-child1', 'fix-child2']);
  });

  it('.map()', () => {
    const result = $('.child').map(function(this: Element) { return this.id; });
    expect(result[0]).toBe('fix-child1');
    expect(result[1]).toBe('fix-child2');
  });

  it('.filter()', () => {
    expect($('.child').filter(function(this: Element) { return this.id === 'fix-child1'; }).length).toBe(1);
  });
