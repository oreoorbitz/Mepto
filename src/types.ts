...
export type AjaxSuccessCallback = (
  data: unknown,
  status: string,
  xhr: XMLHttpRequest
) => void;

export type AjaxErrorCallback = (
  xhr: XMLHttpRequest,
  errorType: string,
  error: Error
) => void;

export type AjaxCompleteCallback = (
  xhr: XMLHttpRequest,
  status: string
) => void;
...
export interface MeptoCollection<T extends MeptoElement = MeptoElement> {
  ...
  fadeTo(duration: number | string, opacity: number, easing?: string, callback?: () => void): MeptoCollection<T>;
  ...
  slideToggle(duration?: number | string, easing?: string, callback?: () => void): MeptoCollection<T>;
  slideToggle(options: AnimationOptions): MeptoCollection<T>;
  animate(properties: CSSProperties, duration?: number | string, easing?: string, callback?: () => void): MeptoCollection<T>;
...
export interface MeptoStatic {
  ...
  // Utilities
  each<T>(collection: T[] | Record<string, T>, callback: (index: string | number, item: T) => boolean | void): T[] | Record<string, T>;
  ...
  get(url: string, data?: PlainObject | string, success?: AjaxSuccessCallback, dataType?: string): XMLHttpRequest;
  post(url: string, data?: PlainObject | string, success?: AjaxSuccessCallback, dataType?: string): XMLHttpRequest;
  ...
  // Events
  Event(type: string, properties?: Record<string, unknown>): Event;
  proxy(fn: (...args: unknown[]) => unknown, context: unknown): (...args: unknown[]) => unknown;
